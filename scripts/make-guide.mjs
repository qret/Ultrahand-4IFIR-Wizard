...
//
// Правило переноса `sha` тонкое: он переставляется, только если ТРОНУЛИ САМ ПЕРЕВОД.
// Иначе правка русского честно оставит английский помеченным как отставший — а если
// переставлять `sha` всегда, метка будет вечно зелёной и не значить ничего.
  ...
  for (const lang of LANGS) {
    ...
    if (lang !== SRC) {
      ...
      if (!page.generated && existsSync(srcPath)) {
        ...
        // Перевод не трогали — `sha` остаётся прежним, и STATUS честно скажет «ОТСТАЛ».
        const sha = prev && prev[2] === self ? prev[1] : bodyHash(readFileSync(srcPath, 'utf8'))
