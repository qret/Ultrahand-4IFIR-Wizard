...
function walk(node, path = [], inMicro = false) {
  ...
  if (node.gate?.offset != null) {
    if (!byOffset.has(node.gate.offset)) problems.unknownOffset.push({ item: node.id, offset: node.gate.offset })
    else own.push(node.gate.offset)
  }
