...
function emitBackup(item, lines) {
  ...
  for (const rev of ['mariko', 'erista']) {
    ...
    // Значок удержания снят 30.08.2026 по решению оператора: он видит экран списка копий,
    // и подсказка там есть. Моё возражение строилось на том, что внутри не написано ничего;
    // оператор это опроверг снимком. `;hold=true` остаётся — удаление необратимо.
    ...
    // ПОДСКАЗКА СЛОВАМИ СНЯТА 30.08.2026 — ОСТАЛСЯ ЗНАЧОК. Решение оператора.
    //
    // Строка «Pick a copy, then hold A to delete it.» стояла под пунктом и отвечала
    // на «что» и «когда», тогда как значок отвечает только на «как». Но отступ до неё
    // был шесть пикселей, а подсветка выделенного пункта рисуется ВЫШЕ его строки —
    // и накрывала верх текста полосой (`docs/NOTES.md` №112, тот же класс). Оператор
    // увидел это на снимке экрана и попросил убрать строку.
    //
    // ЗНАЧОК ОСТАВЛЕН НАМЕРЕННО, вопреки буквальной просьбе, и вот почему. Он виден
    // не только на входе: имя секции движок показывает ЕЩЁ И ЗАГОЛОВКОМ НАД СПИСКОМ
    // КОПИЙ. То есть внутри, где человек стоит на копии и держит A, значок — сейчас
    // единственное, что говорит «здесь удерживают». Строка списка текста не показывает
    // вовсе, `;footer=` там игнорируется, а `set-footer` адресован родителю (NOTES №115).
    // Убрать и строку, и значок значило бы оставить экран удаления совсем без
    // предупреждения: короткое нажатие просто ничего не делает, и это читается
    // как поломка. Это же отменило бы решение №163, принятое после двух заходов.
    //
    // Если значок всё же не нужен — это отдельное решение, и принимать его надо
    // зная, что взамен не остаётся ничего.
...
if (kipRows.length) {
  ...
  for (const rev of ['mariko', 'erista']) {
    ...
    emitPreviewPage(`service/restore-${rev}.ini`, {
      ...
      chooser: [
        `[*Backup?${rev}]`, ';mode=option', ';grouping=split',
        `file_source ${dir}/*.ini`,
