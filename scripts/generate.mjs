...
function emitBackup(item, lines) {
  ...
  for (const rev of ['mariko', 'erista']) {
    ...
    // УДАЛЕНИЕ — ОДИН ПУНКТ-СЕЛЕКТОР, БЕЗ ЗНАЧКА, И ЭТО НЕ ЗАБЫВЧИВОСТЬ.
    //
    // Значок «удерживать A» обязан стоять там, где A действительно удерживают.
    // Здесь удерживают НА СТРОКЕ КОПИИ внутри списка — а поставить его туда
    // невозможно: подпись строки файлового списка это имя файла дословно
    // (`main.cpp:3937-3944`), `;footer=` в списках движок молча игнорирует,
    // а `set-footer` адресован родительскому пункту, не строке.
    //
    // На самом пункте значок стоял и был снят по указанию оператора: там A не держат,
    // там нажимают. Обещание не совпадало с действием.
    //
    // Промежуточную страницу с одним пунктом внутри пробовали — получилось два
    // одинаковых меню подряд, «Delete backup» внутри «Delete backup». Откачено.
